...
v razmerju s številom vozlišč. Iz tega aspekta je bolj zanimiva konstrukcija Mycielskega.
...
Graf $G_k$ zgradimo tako, da pogledamo končno projektivno ravnino reda $k$, torej projektivno ravnino s $k + 1$ točkami. Za vozlišča vzamemo vse urejene pare točk in premic $(p, L)$, za katere velja,
...
različni in da $p$ leži na $L'$. Tedaj velja, da $G_k$ ne vsebuje trikotnikov in da je $\alpha(G_k) \leq k + 1$. Ker ima graf $(k^2 + k + 1)(k + 1)$ vozlišč, je posledično $\chi(G_k) \geq k^2 + k + 1$.
Če ima $G_k$ $n$ vozlišč, je torej $\chi(G_k) = \Omega(n^{\frac{2}{3}})$, kar je občutno večje kromatično število kot v prejšnjih konstrukcijah in kaže, da je lahko graf globalno res precej kompleksen
